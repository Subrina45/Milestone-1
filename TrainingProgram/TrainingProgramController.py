...
from TrainingProgram.TrainingProgramModel import TrainingProgramModel


class TrainingProgramController:

    def __init__(self, path_to_db):
        self.fields = ["Course ID", "Course Name", "Subject Area", "Date", "Time"]
        self.model = TrainingProgramModel(r"" + path_to_db)
    ...
    def update_record(self, id, elements):
        ...
        values.append(id)
        record = tuple(values)
        self.model.update_trainingProgram(record)

    def delete_record(self, id, frame, button):
        self.model.delete_trainingProgram(id)
        frame.destroy()
        button.destroy()